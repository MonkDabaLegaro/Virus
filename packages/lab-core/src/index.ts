...
const transitions: Record<LabAction, LabState> = {
  prepare: 'prepared', detect: 'detected', contain: 'contained', remediate: 'remediating', restore: 'clean'
};
...
export class LabRegistry {
  ...
  create(scenarioId: string, scenarioName: string): LabSession {
    ...
    const lab: LabSession = {
      id: randomUUID(), scenarioId, scenarioName, state: 'idle', createdAt: now, updatedAt: now, vmName: null, snapshot: null
    };
  ...
  transition(id: string, action: LabAction): LabSession | null {
    ...
    const updated: LabSession = {
      ...
      vmName: action === 'prepare' ? 'analysis-win-01' : current.vmName,
      snapshot: action === 'prepare' ? 'clean-baseline' : current.snapshot,
